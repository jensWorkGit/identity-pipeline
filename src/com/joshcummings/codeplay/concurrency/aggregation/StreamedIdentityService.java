...
public class StreamedIdentityService implements IdentityService {
    ...
    public Identity getOne(Predicate<Identity> p) {
        try {
            return es.submit(() -> verifiedIdentities.parallelStream().filter(p).findAny().get()).get();
    ...
    public List<Identity> search(Predicate<Identity> pred) {
        try {
            return es.submit(() -> verifiedIdentities.parallelStream().filter(pred).collect(Collectors.toList())).get();
        } catch (ExecutionException | InterruptedException e) {
            Thread.currentThread().interrupt();

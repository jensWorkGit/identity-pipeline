...
public class ThreadSafeIdentityService implements IdentityService {
    ...
    public Identity getOne(Predicate<Identity> p) {
        try {
            return pool.submit(() -> verifiedIdentities.parallelStream().filter(p).findAny().get()).get();
    ...
    public List<Identity> search(Predicate<Identity> pred) {
        try {
            return pool.submit(() -> verifiedIdentities.parallelStream().filter(pred).collect(Collectors.toList()))
                       .get();
        } catch (ExecutionException | InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private Optional<MergeCandidate> scoreMatch(Identity incoming, Identity existing) {
        int score = 0;
        if (existing.getEmailAddress() != null && existing.getEmailAddress().equals(incoming.getEmailAddress())) {
            score += 50;
        }
        if (existing.getPhoneNumber() != null && existing.getPhoneNumber().equals(incoming.getPhoneNumber())) {
            score += 15;
        }

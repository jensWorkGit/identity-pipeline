...
/**
 * An identity pipeline that coordinates asynchronous dependencies using CompletableFuture.
 */
public class CompletableFutureIdentityPipeline {
    ...
    public CompletableFutureIdentityPipeline(MalformedIdentityRepository malformed, IdentityReader identityReader,
    ...
    public void process(InputStream input) {
        StreamSupport.stream(new IdentityIterable(input, identityReader).spliterator(), true).forEach((identity) -> {
            ...
            try {
                CompletableFuture<Void> address = CompletableFuture.runAsync(() -> validateAddresses(identity), pool);

                CompletableFuture<Void> phoneNumber = CompletableFuture
                        .runAsync(() -> phoneNumberFormatter.format(identity), pool);

                CompletableFuture<Void> email = CompletableFuture.runAsync(() -> emailFormatter.format(identity), pool);

                CompletableFuture.allOf(address, phoneNumber, email).thenRunAsync(() -> {
